...
interface MenuProps<T> extends AriaMenuProps<T>, Omit<AriaMenuTriggerProps, "children"> {}
...
export function MenuHeader(props: AriaHeaderProps) {
	return (
		<AriaHeader
			{...props}
			className='mb-1 px-2 py-1 font-medium text-sm'
		/>
	);
}
...
const menuItemStyles = tv({
	...
	base: ["flex h-8 cursor-default items-center gap-2 rounded px-2 text-sm", "disabled:text-muted-foreground"],
	variants: {
		color: {
			default: ["text-foreground", "hover:bg-primary hover:text-primary-foreground"],
...
export function MenuItem({ color = "default", ...props }: MenuItemProps) {
	return (
		<AriaMenuItem
			{...props}
			className={menuItemStyles({ color })}
		/>
	);
}
